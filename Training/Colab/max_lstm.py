...
# Configuration
DATA_DIR = '/content/drive/MyDrive/Lab/Sequenced_partial/'
SEQUENCE_LENGTH = 10
BATCH_SIZE = 1024
EPOCHS = 200
...
def load_file(filename):
    ...
    df = pd.read_csv(filepath, usecols=['XA', 'YA', 'ZA', 'XG', 'YG', 'ZG'])
    return df.values, action_number
...
def create_model(input_shape, num_classes):
    ...
    model = Sequential([
        ...
        BatchNormalization(),
        Dropout(0.2),
        LSTM(256, kernel_initializer='he_normal'),
        BatchNormalization(),
        Dropout(0.2),
        Dense(128, activation='relu', kernel_initializer='he_normal'),
        BatchNormalization(),
        Dense(num_classes, activation='softmax', dtype='float32')  # Output layer as float32
    ])
